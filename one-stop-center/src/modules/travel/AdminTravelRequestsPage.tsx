...
import { Calendar, Filter, Loader2, Search, UserRound, CheckCircle2, XCircle } from 'lucide-react'
import type { TravelRequestStatus, TravelRequestSummary } from '@/services/travelService'
import { fetchAllTravelRequests, updateTravelRequestStatus } from '@/services/travelService'
...
const statusColors: Record<TravelRequestStatus, string> = {
...
export default function AdminTravelRequestsPage() {
  const [requests, setRequests] = useState<TravelRequestSummary[]>([])
  const [filtered, setFiltered] = useState<TravelRequestSummary[]>([])
  ...
  const handleStatusChange = async (id: string, status: TravelRequestStatus) => {
    setUpdatingId(id)

    // Optimistic UI update
    setRequests((prev) => prev.map((r) => (r.id === id ? { ...r, status } : r)))

    const { error } = await updateTravelRequestStatus(id, status)
    setUpdatingId(null)
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-3xl bg-white/80 p-6 shadow-card">
        <div>
          <nav className="text-sm text-text-muted">
            Home / Admin / <span className="text-brand.violet">Travel Approvals</span>
          </nav>
          <h1 className="mt-2 text-3xl font-semibold text-charcoal">Travel Approvals</h1>
          <p className="text-sm text-text-muted">
            Review and approve travel requests submitted by staff.
          </p>
        </div>
      </div>
      ...
      <div className="rounded-3xl border border-card-border bg-white/80 p-4 shadow-card">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="relative flex-1 min-w-[220px] max-w-md">
            ...
            <input
              type="text"
              placeholder="Search by staff name, department, or destination..."
              value={search}
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value as TravelRequestStatus | 'ALL')}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-card-border bg-brand.sand/30 text-xs uppercase tracking-wider text-text-muted">
                  ...
                  <th className="px-3 py-2 text-left">Travel Dates</th>
                  <th className="px-3 py-2 text-left">Status</th>
                  <th className="px-3 py-2 text-right">Total Meals (RM)</th>
              ...
              <tbody className="divide-y divide-card-border">
                {filtered.map((req) => (
                  <tr key={req.id} className="hover:bg-brand.sand/30">
                    ...
                    <td className="px-3 py-3 align-top">
                      <p className="font-medium text-charcoal">{req.destination}</p>
                      {req.reason && (
                        <p className="mt-0.5 text-xs text-text-muted line-clamp-2">{req.reason}</p>
                      )}
                    </td>
                    ...
                    <td className="px-3 py-3 align-top text-right text-sm font-semibold text-charcoal">
                      {req.totalMealAllowance != null ? req.totalMealAllowance.toFixed(2) : '-'}
                    </td>
                    <td className="px-3 py-3 align-top">
